...
class _LoginPageState extends State<LoginPage> with SingleTickerProviderStateMixin {
  ...
  // إضافة وظيفة للتعامل مع الخروج من الصفحة
  Future<bool> _onWillPop() async {
    // عرض مربع حوار للتأكيد قبل الخروج
    final shouldPop = await showDialog<bool>(
      context: context,
      builder: (context) {
        return AlertDialog(
          title: const Text('هل تريد الخروج من التطبيق؟', textDirection: TextDirection.rtl),
          content: const Text('سيتم إغلاق التطبيق بالكامل.', textDirection: TextDirection.rtl),
          actions: [
            TextButton(
    ...
    // إذا كانت النتيجة true، قم بإغلاق التطبيق
    if (shouldPop == true) {
      SystemNavigator.pop(); // إغلاق التطبيق بالكامل
      return false; // لا يتم تنفيذ العملية الافتراضية للخروج
    }
    return false; // البقاء في الصفحة الحالية
  }
    ...
    try {
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        if (user['password'] == passwordController.text.trim()) {
          ...
          await Provider.of<AuthProvider>(context, listen: false).loginWithUUID(uuid);
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      canPop: false, // منع العودة التلقائية
      onPopInvoked: (didPop) {
        if (!didPop) {
          _onWillPop(); // استدعاء الدالة التي تتعامل مع محاولة الخروج
        }
      },
